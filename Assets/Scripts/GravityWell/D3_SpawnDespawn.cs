...
public class D3_SpawnDespawn : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && gameObject.GetComponent<D3_PlayerController>().canSpawn && onSpawn)
        {
        ...
        else if (Input.GetMouseButtonDown(0) && !onSpawn)
        {
            ...
            onSpawn = true;
            gameObject.GetComponent<D3_PlayerController>().startedWhirl = false;


            Destroy(spawnedWell);
            gameObject.GetComponent<D3_PlayerController>().Launch();
