...
public class modifiedPortal : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.transform.tag == "Player")
        {
            ...
            if (pC.getState() != PlayerController.PlayerState.Teleporting)
            {
                ...
                selectExit();
                StartCoroutine(WaitABit(other, other.transform.position, actualExit.transform.position + new Vector3(0, 0, 100)));
    ...
    private void selectExit()
    {
        ...
        if(rand<10)
        {
            actualExit = exitPortal[1];
        }
        else if (rand > 10 && rand< 60)
        {
            actualExit = exitPortal[0];
        }
        if (rand > 60)
        {
            actualExit = exitPortal[2];
        }
    }
}
